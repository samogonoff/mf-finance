...
> Статус: **черновик спеки, реализация не начата.**
> Дата: 2026-06-25. Ветка под разработку: `add-plan`.
...
**Открытые вопросы:**
- ✅ Q1 (креды) — **закрыто 25.06:** OLAP-источник МП = тот же сервер **FinDWH**,
  что у ВГО-отчёта; переиспользуем `MSSQL_PREMASTER_*` (не плодим `PLANS_OLAP_*`).
  Остаётся уточнить лишь **имя объекта** (вьюха/таблица `ALL_view_МП`,
  `Источник_МП_штрафы` в FinDWH) — снять через `cmd/mssql-probe`; до этого
  `PLANS_MOCK=1`.
