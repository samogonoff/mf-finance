...
- [x] «Сумма+самовывоз» как база сравнения (§6a) — СНЯТО со скоупа ответом финблока §12 п.7 (27.08.2026): публикуем и сравниваем по `Параметр='ПРОДАЖИ'`, «курс П.М» и «индекс самовывоза» остаются неопределёнными и больше не нужны
...
- [x] База порога 106 % для ФОТ (§5) и «оборотные» магазины (§12 п.13) — закрыто ответом финблока 27.08.2026: порог 106 % считается от плана продаж ТЕКУЩЕГО месяца, удельный вес применяется к продажам БЕЗ НДС и задаётся по стране, детализация по магазинам — в пропорции ср. факта за последний закрытый квартал (`bulkPayroll` переписан, 4 теста); аренда на этапе 1 — только факт предыдущего месяца, оборотная часть под явным `rent_turnover`
- [ ] ⏳ Аренда этап 2 — расчёт от условий договора там, где есть привязка (ответ §12 п.13: «надо будет перестроить»); вне скоупа §5

### [x] Фаза 3a. Ответы финблока по Розница §12 (27.08.2026)
Разбор ответов и решения — `SPEC.md` §28.9. Кодом закрыто:
- [x] `bulkPayroll` — фонд страны от продаж БЕЗ НДС + детализация по магазинам в пропорции ср. факта за последний ЗАКРЫТЫЙ квартал (календарь, §4.4); порог 106 % стоит на базе начисления (формула §5), база по умолчанию — план текущего месяца (`plan_current_month`); магазины без истории считаются напрямую и несут пометку
- [x] `bulkRent` — этап 1: только факт аренды предыдущего месяца; оборотная часть включается явным `rent_turnover=true` (переходный режим до расчёта от условий договора)
- [x] `bulkSalesIndex` — предупреждение, если индекс задан только переопределениями: по §12 п.11 значение утверждается на уровне страны
- [x] `RetailPlanVersionLabel` — формат метки VERSION `TAKT-2026-07-v003` (§12 п.8 закрыт нашим решением), лексикографический порядок = хронологический
- [x] `migrations/0039_plans_tz_answers` — шаг «Финансист» включён и переставлен между 1.2 и 1.3 (§12 п.4); правила публикации розницы включены (§12 п.7)
- [x] Фронт: база порога и пояснение формулы ФОТ, чекбокс оборотной части аренды, подсказка про страновой уровень величин (`retail-form/[cardId].vue`, `useRetail.ts`)
- [x] Исключено из скоупа: §12 п.6 (`CodePL NOT IN ('50','93','01')`), база «Сумма+самовывоз» (§6a)
...
- [x] Розница: правила `publish_mapping` включены ответом §12 п.7 (миграция 0039) — параметр `ПРОДАЖИ`, нац. валюта; BYN-пару заполняет ETL BI (§12 п.2). Запись по-прежнему под `PLANS_PUBLISH_ENABLED`
- [ ] ⏳ МП: заполнить `publish_mapping` ответами BI (МП §12 п.1/2 — «Параметр», агрегат vs детализация) и включить `PLANS_PUBLISH_ENABLED=1`
- [ ] ⏳ Уникальный индекс приёмника по (Параметр, Страна, КодЦФО, КодPL, Дата) — остаток §12 п.2, на стороне BI; до него идемпотентность держится DELETE+INSERT в транзакции
