...
## ✅ Факты по ПТИР и ТЕКС (выполнено 2026-06-02, период с 2024/2025)

- **ПТИР (`9731039708`)** из наших РФ/РБ ЮЛ рассчитывается **только с ТД**
  (`6950135110`): 164 строки, период 2024-01…2026-04. Распределение `ICO`:
  **все 164 строки `ICO=1`** — метка ВГО проставлена корректно. С другими
  нашими ЮЛ (Марк Формэль/Формэль/ТЕКС/Филиал/ИП) расчётов нет.
- Прочие контрагенты ПТИР — внешние (`7702070139` Сбербанк и т.п.), все `ICO=0`.
- **ТЕКС (`5031159833`)** — тоже **только с ТД** (`6950135110`): 9329 строк, `ICO=1`,
  2025-02…2026-06.
- **Вывод:** ТД («ООО Торговый Дом „Марк Формэль“») — центральный хаб группы;
  периферийные РФ-ЮЛ ведут внутригрупповые расчёты только через него. «Везде
  только ТД» — это реальная структура, не ограничение кода.
- **Корректировка по #4:** для ПТИР метка `ICO` верна (всё =1), значит корнем
  «данных нет» был не флаг, а отсутствие ПТИР в выборке/импорте. Union-фильтр
  (ico=1 ИЛИ наш контрагент) остаётся правильной страховкой на случай пар с `ICO=0`.
- **Договоры ПТИР** (счёт 60 `CrSubconto1` + 62 `DrSubconto2`, период с 2023):
  сырой резолв субконто даёт мусор — документы («Оказание производственных услуг
  00БС-…», десятки), контрагентов («ИНГОССТРАХ СПАО»). После фильтра
  `NOT LIKE '%00БС%'/'%ТДБП%'/'Оказание%'/'Реализаци%'/'Поступлени%'` — ~20 чистых
  договоров: «20/09 от 20.09.2021», «Договор поставки …», «Дополнительное
  соглашение …», «Счёт-оферта …», «основной договор». Подтверждает: нужна
  **эвристика-фильтр**, а не слепой резолв одной позиции субконто.
...
## ✅ Мост «договор → срок оплаты» (реализовано в коде, 2026-06-22)

Срок/отсрочка по договору живут в `Payments.dbo.Docs` (`Delay`, `PaymentDate`),
ключ — **сам договор** (`Docs.ID` = чистый GUID договора). Договор лежит в субконто
Premaster (1С-ссылка `{"#",тип,N:hex}`), её надо конвертировать в чистый GUID.

**Формула (= штатная UDF `dbo.Convert_IDRRefToGUID`, инлайн в коде `idrrefSQLToGUID`),
`h` = последние 32 символа ссылки до `}`:**

```
SUBSTRING(h,25,8)+'-'+SUBSTRING(h,21,4)+'-'+SUBSTRING(h,17,4)+'-'+SUBSTRING(h,1,4)+'-'+SUBSTRING(h,5,12)
```

Подаётся как есть (h = SQL-binary hex). Проверено: совпадает с UDF 1:1 (79/79).
Кросс-БД джойн к `Docs` требует `COLLATE DATABASE_DEFAULT` (`Docs.ID` = `Cyrillic_General_BIN`).

**Что НЕ работает:** мост по `Premaster.DocID` (это бух-документ Реализация/Поступление,
другой объект 1С) — `converted DocID → Docs/Debt_arh` = 0 совпадений. Только через договор-субконто.

**Покрытие (проверено вживую):**
- Дебиторка 62 (договор = субконто #2): ТЕКС 79/79 = 100% матч в `Docs`, срок проставляется.
- Кредиторка 60/76 (договор = субконто #1 по ПТИР): позиция **не универсальна** —
  у ТЕКС субконто #1 на 60 = контрагент, а не договор. Поэтому в коде договор
  резолвится **только для 62**; 60/76 → группа «без договора» до получения карты
  «счёт→вид субконто» (или `DimSubkonto`) от аналитика.
- Срок есть не у всех договоров (ВГО ~14-29%); где не заведён — поле пустое.

Реализация: `repo_premaster.go` (Report: договор в `GROUP BY` + JOIN Objects/Docs;
Drilldown: фильтр по договору), `service.go` (`BuildReport` заполняет срок/просрочку).
