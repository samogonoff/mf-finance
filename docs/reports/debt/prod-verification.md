...
## ✅ Результаты прогона на проде (2026-06-24, server 10.10.6.15)

- **A. Premaster1C vs GLMF — РАСХОДЯТСЯ.** MF·2025-01: Premaster1C = **101 строка** /
  SUM currency 49 649; GLMF = **1 112 775** / −33.4М. Стабильно (2 прогона = 101).
  Premaster1C полон по 2026 (~5М/мес), но по части срезов 2025 почти пуст (вероятно
  архивировано в `Premaster1CHistory`, 871M, субконто-колонки на месте).
  → **GLMF — единственный полный+свежий+классифицированный источник.**
- **C. Договора в GLMF НЕТ.** `DocName1C` по счёту 62 = документы («Расходная
  накладная ТМЦ», «Возвратная накладная»), не договоры. Субконто-колонки в GLMF
  отсутствуют. Договор есть только в субконто Premaster1C/History → `Objects.Name`.
- **D.** ICO/договор-пробы по Premaster пусты — следствие неполноты Premaster1C по
  тем срезам (не ошибка резолва).

**Вывод:** «нужны договоры» + «быстро в CH» при том, что GLMF полный но без договора,
а Premaster с договором но неполный/волатильный → нужно **свести договор и каноничные
числа в один источник**. Два пути:
1. **(рекомендуется) Upstream:** попросить аналитика (Васянович В.) добавить в
   `GLMF_exec` колонку договора — он там УЖЕ джойнит `Objects` для `DocName1C`,
   добавить резолв субконто (62→`DrSubconto2`, 60/76→`CrSubconto1`) — несколько строк.
   Тогда GLMF несёт всё → ETL GLMF→CH тривиальный.
2. **(на нашей стороне) Join в ETL:** числа+класс. из GLMF, договор — LEFT JOIN
   Premaster1C по `DocID`+`RwNm`/`Num` → субконто → `Objects`. Тяжелее, зависит от
   полноты Premaster по периоду отчёта.
