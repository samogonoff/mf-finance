...
func ValidateMpForm(in MpValidationInput) []MpIssue {
	...
	for _, p := range sortedPlatforms(in.Platforms) {
		...
		// МП-11: наименование статьи в форме совпадает со справочником по коду.
		// Контроль конфликта кодов 52 и 54 (ТЗ §8.1: в справочнике 54 —
		...
		for _, l := range spec {
			...
			if want, ok := in.PLNames[l.CodePL]; ok && want != "" && want != l.Name {
				add("МП-11", MpBlocking, false,
					"статья %d в форме названа «%s», а в справочнике — «%s»", l.CodePL, l.Name, want)
			}
		}
	}
