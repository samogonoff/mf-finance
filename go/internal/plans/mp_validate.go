...
func ValidateMpForm(in MpValidationInput) []MpIssue {
	...
	// по тому же коду. Проверка уровня ФОРМЫ, а не площадки: наименование статьи от
	// площадки не зависит, и повторять одно замечание по каждой площадке — шум.
	//
	// Блокируют только коды, по которым ТЗ зафиксировал СОДЕРЖАТЕЛЬНЫЙ конфликт
	// (§8.1, §12 п.10): 54 — «Аренда помещений (стоянки)» в справочнике против
	// «Складская логистика» в форме, 52 — «Грузоперевозки развоз и перемещение»
	// против «Транспортная логистика». Там расходится СМЫСЛ статьи, и свод по
	// группам PL исказится. Остальные расхождения — разная формулировка одного и
	// того же («Эквайринг» против «Банковские расходы / эквайринг»): это повод
	// свериться с НСИ, но не причина останавливать весь плановый цикл.
	for _, l := range spec {
		if l.CodePL == 0 || !l.CostLine {
			continue
		}
		want, ok := in.PLNames[l.CodePL]
		if !ok || want == "" || want == l.Name {
			continue
		}
		level, needsWhy := MpWarning, false
		msg := fmt.Sprintf("статья %d в форме названа «%s», а в справочнике — «%s» — сверить формулировку с НСИ",
			l.CodePL, l.Name, want)
		if mpConflictingPL(l.CodePL) {
			level = MpBlocking
			msg = fmt.Sprintf("конфликт кода %d: в форме «%s», в справочнике «%s» — при своде статья попадёт в чужую группу PL (ТЗ §8.1, §12 п.10)",
				l.CodePL, l.Name, want)
		}
		out = append(out, MpIssue{
			Code: "МП-11", Level: level, Block: l.BlockType, Message: msg, NeedsWhy: needsWhy,
		})
	}
...
// mpConflictingPL — коды статей, по которым ТЗ зафиксировал содержательный
// конфликт со справочником (§8.1): 52 (транспортная логистика против
// «грузоперевозки развоз и перемещение») и 54 (складская логистика против
// «аренда помещений»). Решение НСИ по §12 п.10 — привести наименования к
// справочнику либо завести отдельные коды для логистики МП.
func mpConflictingPL(codePL int) bool {
	return codePL == 52 || codePL == 54
}
