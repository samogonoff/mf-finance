...
//
// Состав: 18 магазинов РБ — реальные CodeCFO и адреса из dir_cfo (миграция 0028),
// поэтому строки формы совпадают со справочником ЦФО. Набор специально
// разнородный, чтобы включались все ветки расчёта:
...
// Факт — BYN, помесячно 2025-01…2026-06; масштаб ≈ 120–420 тыс. BYN/мес.
...
type mockRetailStore struct {
	...
	open      string
	close     string
	// base — «средний месяц» 2025 года в BYN; факт получается умножением на
	// сезонный коэффициент месяца и годовой уплотнитель.
	base float64
}

// mockRetailStores — 18 магазинов РБ (CodeCFO и адреса — из dir_cfo, 0028).
func mockRetailStores() []mockRetailStore {
	return []mockRetailStore{
		{100, "б-р Шевченко, 9", "Минск", "1641", LFLYes, "стрит", "B", "Смолер О.В.", 133.7, "2019-04-12", "", 168000},
		{113, "ТЦ Манеж, Вильнюсское шоссе, 1", "Полоцк", "1669", LFLYes, "ТЦ", "A", "Смолер О.В.", 148.1, "2019-08-01", "", 152000},
		{114, "ул.Ленина, 57", "Береза", "2246", LFLYes, "стрит", "A", "Смолер О.В.", 124.1, "2020-03-14", "", 121000},
		{115, "м-н Первомайский, 3", "Светлогорск", "1676", LFLYes, "стрит", "B", "Смолер О.В.", 164, "2019-11-02", "", 134000},
		{117, "ул.Ленина, 13", "Кобрин", "1721", LFLYes, "стрит", "B", "Смолер О.В.", 165.8, "2020-06-20", "", 129000},
		{119, "ул.Сумченко, 63", "Осиповичи", "1794", LFLYes, "стрит", "B", "Ковалёва И.П.", 112.3, "2020-09-05", "", 118000},
		{121, "пр-т Ленина, 22", "Жодино", "1824", LFLYes, "стрит", "A", "Ковалёва И.П.", 141.6, "2021-02-13", "", 146000},
		{122, "ТЦ Простор, Каменногорская, 3", "Минск", "2023", LFLYes, "ТЦ", "B", "Смолер О.В.", 126.4, "2021-05-22", "", 158000},
		{123, "ТЦ DANAMOL, П.Мстиславца, 4", "Минск", "2083", LFLYes, "ТЦ", "A", "Смолер О.В.", 283.7, "2021-09-18", "", 287000},
		{124, "ТЦ ГРИН, ул.Притыцкого, 156", "Минск", "2358", LFLYes, "ТЦ", "A", "Смолер О.В.", 411.4, "2022-03-05", "", 418000},
		{127, "ул.Ленина, 53", "Рогачев", "2092", LFLYes, "стрит", "B", "Ковалёва И.П.", 162.6, "2022-06-11", "", 124000},
		{128, "ТЦ Планета GREEN, Островского, 5", "Могилев", "2093", LFLYes, "ТЦ", "B", "Ковалёва И.П.", 180, "2022-08-27", "", 171000},
		{131, "ул.Замковая, 19", "Витебск", "2150", LFLYes, "стрит", "A", "Ковалёва И.П.", 401.6, "2023-04-08", "", 264000},
		// «до года» — открыты 2025-09: факт есть не за все месяцы, W-01/W-02 не применяются (§6).
		{133, "ул.Мицкевича, 104Б", "Новогрудок", "2312", LFLUnder1, "стрит", "B", "Ковалёва И.П.", 95.2, "2025-09-12", "", 96000},
		{134, "ул. М. Октябрьской, 73", "Лиозно", "2177", LFLUnder1, "стрит", "C", "Ковалёва И.П.", 126.2, "2025-09-26", "", 88000},
		// «новые» — открыты 2026-03: LFL тактич. и % вып. не рассчитываются (§4).
		{136, "ул.Водопьянова 19", "Вилейка", "2277", LFLNew, "стрит", "A", "Ковалёва И.П.", 207.8, "2026-03-02", "", 0},
		// «ххх» — магазин есть, KLIENT_ID ещё нет (§2: 10 новых магазинов РБ).
		{330, "ТЦ Экспобел, Дражня", "Минск", "", LFLNoID, "ТЦ", "B", "Смолер О.В.", 190.5, "2026-04-15", "", 0},
		// Закрытый — DateClose до начала периода: V-03/V-09.
		{130, "ТЦ ГРИН, Шабаны", "Минск", "2098", LFLClosed, "ТЦ", "B", "Closed", 79, "2021-07-01", "2026-02-28", 74000},
	}
}
...
// Stores — магазины фикстуры. Страна у всех BY: ТЗ §1 требует независимых форм
// по странам, и mock покрывает одну — РБ (191 магазин в проде, 18 в фикстуре).
func (s *MockRetailSource) Stores(_ context.Context, country string) ([]RetailStore, error) {
	...
	out := make([]RetailStore, 0, len(mockRetailStores()))
	for _, m := range mockRetailStores() {
		out = append(out, RetailStore{
			...
			City: m.city, Country: "BY", CodeFOX: "FOX" + itoa(int64(m.code)),
			Ploschad: m.area, StoreType: m.storeType, DateOpen: m.open, DateClose: m.close,
			Stage: "действующий", CompanyMF: "F", Channel: "Розница", Category: m.category,
			LFLStatus: m.lfl, RegManager: m.rm, Manager: m.rm, PLAnalytic: "Розница РБ",
		})
	}
