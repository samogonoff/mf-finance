...
from app import calc_structure, insights, margin
...
# Было шесть — по минимальному пути «разрез → провал в подгруппу → структура →
# сравнение периодов → вывод». Этого хватало, чтобы НАЙТИ отклонение, но не
# чтобы его проверить: агент останавливался на первой же аномалии, и разбор
# выходил сухим (08.09.2026, жалоба заказчика). Десять дают запас на сравнение
# с соседним периодом и соседней группой — то, что отличает разбор от сводки.
MAX_STEPS = 10

# Десять шагов по два вызова. Потолок здесь защищает витрину от зацикливания, а
# не экономит: один вызов — это проекция `margin.dashboard()` из кэша.
MAX_TOOL_CALLS = 20
...
#
# 180 с — предел разумного ожидания человека у экрана, а не техническое
# ограничение (исследование фоновое, страница показывает прогресс по шагам).
# Поднято с 120 вместе с MAX_STEPS: десять шагов в 120 с не укладывались, и
# бюджет сворачивал разбор раньше, чем агент успевал перепроверить вывод.
# Меняется через COST_LLM_AGENT_BUDGET и из админки без правки кода.
def _time_budget() -> float:
    from app import llm_settings
    return float(llm_settings.int_value(
        "agent_budget_s", "COST_LLM_AGENT_BUDGET", 180))
...
class _Cache:
    ...
    async def structure(self, filters: dict, by: str | None = None) -> dict:
        """Структура себестоимости по статьям. Отдельный кэш от дашборда:
        запрос другой, а спрашивают один и тот же срез по нескольку раз."""
        key = json.dumps(["structure", filters, by], ensure_ascii=False, sort_keys=True)
        if key not in self._data:
            self._data[key] = await calc_structure.cost_structure(filters, by=by)
            self.sql_calls += 1
        return self._data[key]

    async def stages(self, articul: str, plan_id: str | None = None) -> dict:
        key = json.dumps(["stages", articul, plan_id], ensure_ascii=False, sort_keys=True)
        if key not in self._data:
            self._data[key] = await calc_structure.calc_stages(articul, plan_id=plan_id)
            self.sql_calls += 1
        return self._data[key]
...
TOOL_SPECS: list[dict] = [
    ...
    },
    {
        ...
        "function": {
            "name": "cost_structure",
            "description": (
                "СТРУКТУРА СЕБЕСТОИМОСТИ по статьям источника: материалы основные "
                "и вспомогательные, пошив, раскрой, декоры, вязание — на штуку и "
                "долей в себестоимости, плюс динамика по месяцам. По пошиву и "
                "раскрою отдельно СТАВКА МИНУТЫ норматив против факта: именно она "
                "объясняет удорожание при неизменных минутах. Этим инструментом "
                "отвечают на вопрос «из чего сложился рост себестоимости»: сначала "
                "найди группу через breakdown/drill, потом разложи её здесь. "
                "by задаёт разрез — у какой группы поехала статья."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "by": {
                        ...
                        "enum": ["level01", "level02", "level03", "level04",
                                 "brand_manager", "articul", "model_name"],
                        "description": "Разрез структуры. Без него — только итог и месяцы.",
                    },
                },
                "required": ["filters"],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "calc_stages",
            "description": (
                "ЭТАПЫ КАЛЬКУЛЯЦИИ одного артикула: ПКПСС и КПСС (плановые), ПФКСС "
                "(предварительный факт), ФКСС (факт выпуска, по нему считается "
                "дашборд) — себестоимость и статьи на каждом этапе. Показывает, как "
                "калькуляция менялась от плана к факту и НЕ ПОТЕРЯЛАСЬ ЛИ СТАТЬЯ: "
                "поле «статьи_пропали_на_этом_этапе» — главный признак дефекта "
                "(например себестоимость упала, потому что в ФКСС нет пошива). "
                "Вызывай, когда у артикула странная себестоимость или маржа. "
                "Больше одного этапа есть не у всех артикулов — один этап в ответе "
                "означает, что сравнивать нечего."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "articul": {
                        "type": "string",
                        "description": "Артикул ПОБУКВЕННО, как в данных, со всеми суффиксами.",
                    },
                    "plan_id": {
                        "type": "string",
                        "description": "Номер плана, если нужно сузить до одного плана.",
                    },
                },
                "required": ["articul"],
            },
...
async def _tool_cost_structure(cache: _Cache, args: dict) -> dict:
    filters = _clean_filters(args.get("filters"))
    by = args.get("by") or None
    if by is not None and by not in calc_structure.STRUCTURE_DIMS:
        # Модель иногда просит разрез, которого нет («level05», «country»).
        # Ошибку не возвращаем: разрез — уточнение, а итог по срезу полезен и
        # без него. Иначе шаг тратится впустую.
        by = None
    data = await cache.structure(filters, by=by)
    out = {
        "срез": data.get("срез"),
        "итого": data.get("итого"),
        "по_месяцам": (data.get("по_месяцам") or [])[-MAX_ROWS:],
    }
    if data.get("как_читать"):
        out["как_читать"] = data["как_читать"]
    if data.get("разрез"):
        rows = (data["разрез"].get("строки") or [])[:MAX_ROWS]
        out["разрез"] = {"по": data["разрез"].get("по"), "строки": rows}
    return out


async def _tool_calc_stages(cache: _Cache, args: dict) -> dict:
    articul = str(args.get("articul") or "").strip()
    if not articul:
        return {"ошибка": "нужен артикул: этапы калькуляции сравнимы только внутри одной позиции"}
    plan_id = str(args.get("plan_id") or "").strip() or None
    data = await cache.stages(articul, plan_id=plan_id)
    return {**data, "этапы": (data.get("этапы") or [])[:MAX_ROWS]}
...
_TOOLS = {
    ...
    "cost_structure": _tool_cost_structure,
    "calc_stages": _tool_calc_stages,
...
2. Дальше сужай: `breakdown` и `drill` показывают, где именно сидит проблема,
   ...
   минуты.
3. РАЗЛОЖИ НА СТАТЬИ. Когда группа или период найдены, вызови `cost_structure`:
   он показывает, из чего сложилась себестоимость — материалы, пошив, раскрой,
   декоры — и что именно изменилось. «Себестоимость выросла» без этого шага —
   это не разбор, а пересказ вопроса. Смотри в первую очередь:
   * какая СТАТЬЯ дала рост в BYN на штуку и как изменилась её доля;
   * СТАВКУ МИНУТЫ норматив против факта: минуты у обеих одинаковые, поэтому
     удорожание операции при тех же минутах — это ставка, а не трудоёмкость;
   * «прочее» — часть себестоимости, которую источник не раскладывает. Если
     рост пришёлся на неё, так и скажи: по этим данным статья не определяется.
4. ПРОВЕРЬ КАЛЬКУЛЯЦИЮ. Если у артикула себестоимость или маржа выглядит
   странно, вызови `calc_stages`: он показывает этапы одной позиции (ПКПСС,
   КПСС, ПФКСС, ФКСС) и поле «статьи_пропали_на_этом_этапе». Пропавшая статья —
   типичная причина «себестоимость упала» и «маржа −200%»: это дефект
   калькуляции, а не экономика. Проверяй это ПРЕЖДЕ чем объявлять
   экономический вывод по такой позиции.
5. Делай 4–8 вызовов. Каждый следующий выбирай по тому, что показал
   предыдущий, а не по заранее составленному списку. Не останавливайся на
   первом найденном отклонении: проверь, объясняет ли оно вопрос целиком —
   сравни с соседним периодом или соседней группой, спустись на уровень
   ниже, посмотри позиции. Один вызов и вывод — это не разбор.
6. Когда картина ясна и проверена сравнением — дай ответ.

Правила вывода:
- Пиши ТОЛЬКО по-русски.
- Все числа — из результатов инструментов. Не считай сам, кроме простых
  разностей, и такие помечай словом «разница».
- НАПРАВЛЕНИЕ ИЗМЕНЕНИЯ СВЕРЯЙ С ЧИСЛАМИ. «Выросли с 14,88 до 14,22» —
  это снижение, а не рост (проверено 08.09.2026: модель привела числа
  верно, а направление назвала наоборот). Перед словами «вырос»,
  «упал», «выше», «ниже» посмотри, какое из двух чисел больше.
...
{"answer": "3–6 предложений: главный вывод и насколько он объясняет вопрос",
...
ГЛУБИНА ОТВЕТА. Отвечай развёрнуто: заказчик просит разбор, а не сводку.
- answer — 3–6 предложений: что происходит, где именно сидит причина, насколько
  она объясняет вопрос целиком (и что осталось необъяснённым);
- 4–7 findings. В каждом: наблюдение, число, с чем сравнивается (соседний
  период, соседняя группа, медиана, норматив) и что из этого следует. Не
  ограничивайся констатацией «маржа 53,25 %» — покажи, много это или мало и
  относительно чего;
- обязательно раздели вклад в результат: что дала структура выпуска (сместился
  микс групп/артикулов) и что дала экономика самих позиций. Для второго есть
  `cost_structure`: назови статью и ставку минуты, а не общее «выросла
  себестоимость». Если разделить не получается — скажи прямо, чего не хватило;
- evidence — инструмент, разрез и ключевые числа, по которым вывод проверяется;
- data_quality — что с охватом, незакрытым периодом, покрытием факта;
- 2–4 next_steps.

Единственный жёсткий лимит: весь JSON целиком не длиннее 6000 символов —
за этим он обрывается и теряется целиком. В эти 6000 всё перечисленное выше
влезает свободно, поэтому краткость экономить не нужно: недосказанный ответ
хуже длинного."""
...
async def _chat(messages: list[dict], with_tools: bool) -> dict:
    ...
    payload: dict[str, Any] = {
        ...
        "messages": messages,
        # 8000, а не 4000: развёрнутый разбор (answer 3–6 предложений,
        # до 7 findings со сравнениями) занимает ~7,5 тыс. символов —
        # замер 08.09.2026, это уже около 3,5 тыс. токенов. При 4000
        # ответ упирался в потолок и уходил в починку обрезанного JSON.
        "max_tokens": 8000,
        "temperature": 0.2,
    }
