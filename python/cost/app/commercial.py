...
Полуфабрикаты и мультипаки из базы исключены — см. BASE_CONDITIONS.
"""
...
# «Семья» из измерений убрана 14.08.2026: поле мусорное — значение «0» у 87 176
# калькуляций, «1» у семи (это те самые, что расходятся внутри ключа, миграция
# 0036). Как разрез даёт один сегмент на 100%, как фильтр — шум.
...
# ИНВАРИАНТ: каждое измерение кольца обязано быть и фильтром — иначе клик по
# сегменту (кросфильтрация) некуда записать. Проверяется ниже, после DIMENSIONS.
...
# Клик по сегменту кольца записывается в фильтр того же измерения — значит,
# измерение без фильтра ломает кросфильтрацию. Падаем на импорте, а не теряем
# клик молча.
assert set(DIMENSIONS) <= set(FILTERS), (
    f"измерения кольца без фильтра: {sorted(set(DIMENSIONS) - set(FILTERS))}"
)
...
BASE_CONDITIONS = (
    ...
    # Полуфабрикаты — вне коммерческого анализа (решение заказчика 14.08.2026).
    # У них цена и себестоимость в РАЗНЫХ единицах: S-023 «Аппликация из страз» —
    # отпускная 760 BYN при себестоимости 0,16, то есть наценка в 4700 раз. При
    # 59 калькуляциях в базе они давали 162,9 млн BYN выпуска — больше, чем весь
    # раздел «Мужчинам» (145,4 млн), — и тянули взвешенную маржинальность с
    # 50,92% до 59,77%.
    #
    # IS DISTINCT FROM, а не <>: level01 пуст у 31 721 калькуляции, и обычное
    # сравнение выбросило бы их все вместе с полуфабрикатами.
    "level01 IS DISTINCT FROM 'Полуфабрикаты'",
    # Мультипаки — вне коммерческого анализа (решение заказчика 02.09.2026).
    # Себестоимость пака СОБРАНА из калькуляций одиночек, а они в базе уже есть:
    # оставить пак — значит посчитать их стоимость второй раз, и структура
    # себестоимости раздела «Носки&Колготки» поехала бы. В выпуске пака нет и
    # так — производят одиночки, пак только комплектуют.
    #
    # Условие по СОСТАВУ, а не по колонке витрины: состав правят чаще, чем идёт
    # REFRESH cost_calc_mv, и флаг внутри витрины показывал бы устаревшее
    # состояние до следующего обновления кэша.
    #
    # Именно NOT IN, а не NOT EXISTS с квалификатором: эти условия подставляются
    # и в запросы `FROM cost_calc_mv` (здесь), и в `FROM cost_calc_mv c`
    # (margin.py, _MARGIN_BASE) — одной строкой на оба случая. Квалификатор
    # `cost_calc_mv.model` под алиасом `c` ломал «Маржу выпуска» целиком:
    # invalid reference to FROM-clause entry for table "cost_calc_mv"
    # (02.09.2026). Без квалификатора `model`/`articul` резолвятся во внешней
    # таблице при любом алиасе; внутри NOT EXISTS так нельзя — там их перехватила
    # бы cost_multipack, у которой колонки называются так же.
    #
    # coalesce(…, TRUE): при NULL в модели NOT IN даёт NULL и строка пропала бы,
    # а калькуляция без модели паком быть не может — оставляем.
    "coalesce((model, articul) NOT IN (SELECT model, articul FROM cost_multipack), TRUE)",
)
