...
def _normalize_signs(text: str) -> str:
    return _RANGE_DASH_RE.sub(" … ", text).translate(_MINUS_CHARS)
...
def _verify_numbers(text: str, pool: list[float]) -> list[str]:
    ...
    """
    unverified: list[str] = []
    for raw in _NUM_RE.findall(_normalize_signs(text)):
        ...
        if val == int(val) and abs(val) <= 3000:
            continue
        if not any(abs(val - p) <= max(0.05, abs(p) * 0.01) for p in pool):
...
9. От 3 до 5 наблюдений, каждое — одно предложение до 200 символов.
