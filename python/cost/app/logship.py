"""
Логи раздела «Себестоимость» → stdout + Logstash/ELK.

Тот же контракт, что у Go-API (go/internal/logship) и Nitro-роутов Nuxt
(nuxt/server/utils/logstash.ts):

  * одна JSON-запись на строку (кодек json_lines), поля
    time/level/msg/service/host + свои — чтобы в общем индексе ELK все рантаймы
    кабинета лежали единообразно. service=finance-cost;
...
def log(level: int, msg: str, **fields: Any) -> None:
    ...
    logging.getLogger("cost").log(level, msg, extra={"fields": fields})
...
def setup_logging() -> None:
    ...
    Вызывается один раз из app/main.py при импорте приложения — то есть уже
    ПОСЛЕ того, как uvicorn настроил свои логгеры, поэтому его хендлеры мы
    снимаем и заворачиваем всё на root (иначе access-лог uvicorn остался бы
    текстовым и мимо ELK).
    """
    global _shipper
    ...
    # Свой access-лог (app/main.py) богаче uvicorn'овского — там есть
    # request_id, длительность и пользователь. Гасим INFO-строки uvicorn.access,
    # чтобы каждый запрос не уезжал в ELK дважды; предупреждения остаются.
