...
export default defineNuxtPlugin(() => {
  ...
  if (!user.value) {
    user.value = {
      ...
      // согласование и админку ролей раздела (/cost/admin/roles — там и
      // раздаются права cost:*), иначе половину страниц в этом контуре
      // не проверить.
      //
      // ROLE_ADMIN здесь НЕ НУЖНА, хотя соблазн есть. Она открывает блок
      // «Админ» в сайдбаре кабинета (nuxt/app.vue смотрит isAdmin), а это
      // страницы /admin/users и /admin/bugtracker — они ходят в Go-API,
      // которого в cost-only контуре нет. Получаются две ссылки в никуда.
      // Пробовал 18.08.2026, откатил.
      //
      // Права раздела и роли кабинета — две независимые системы, и путать их
      // не стоит: «Full Admin» в разделе (все cost:*) не делает пользователя
      // админом кабинета, и наоборот. Кнопки внутри раздела гейтятся только
      // правами cost:* — см. useCostPermission.can().
      //
