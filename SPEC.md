...
**Что меняет дизайн (учтено в решениях ниже):** субсчёт-уровень (полный `DrAcc/CrAcc`),
выручка по точным Дт/Кт. **Построчная валюта — ИСКЛЮЧЕНА из скоупа** (транзакционной
валюты долга в FinDWH нет, §10.7): суммы в BYN-консолидации (+USD), без валютного
измерения строки.
...
| B | Источник extract | **базовая `GLMF`** (есть `DateOfLoad`-watermark; view `vGLMFAddUSD` даёт USD, но НЕ несёт `DateOfLoad` → не годится для инкремента). **USD отложен** (нет в базе; в отчёте валюта=BYN, §10.7) — добавить позже join'ом view/CurrencyDaily |
| C | Суммы | **все 6**: WOVAT/WithVAT × BelRub(BYN)/Currency(нац)/USD |
| C2 | **Валюта — ВНЕ скоупа** (descoped) | транзакционной валюты долга в FinDWH нет (§10.7). Строка на (счёт/субсчёт×компания×партнёр); суммы в **BYN-консолидации** (GLMF нативно) + USD. Колонка «Валюта» = BYN. Построчная валюта ТЗ — отложена (вернуться, если аналитик добавит тр. валюту в GLMF) |
...
| Валюта (колонка) | константа **BYN** (консолидация; построчная валюта вне скоупа, §10.7) |
| Суммы (BYN + USD) | `fact_glmf` (BYN-консолидация нативно; USD из vGLMFAddUSD) |
...
7. **⚠ Валюта задолженности (построчно, ТЗ) — КОНКРЕТНАЯ валюта НЕ материализована в FinDWH.**
   ...
   - ❌ `GLMF/Premaster.AmountWOVATCurrency` хранится в **функц. валюте компании**, не в
     ...
     нормализована прочь.
   **РЕШЕНО (descope):** построчную «валюту долга» (USD/EUR/CNY) из FinDWH получить нельзя
   → этот пункт ТЗ **исключён из скоупа**. Суммы — в BYN-консолидации (+USD), колонка
   «Валюта» = BYN. Вернуться, если аналитик добавит транзакционную валюту в GLMF из 1С.
   (Не блокер для реализации.)
