...
7. **⚠ Валюта задолженности (построчно, ТЗ)** — в `Premaster1C`/`GLMF` явной колонки
   валюты НЕТ (только `AmountWOVATCurrency` без кода; валютных субсчетов у ТД не видно;
   таблицы валют в FinDWH нет — курсы в `Checks.CurrencyDaily`). Установить источник:
   валюта договора (1С/`Docs`)? субконто валютных счетов? вывод из `ExRate`? — вопрос к
   аналитику/разведке. Без него построчная валюта не реализуема.
