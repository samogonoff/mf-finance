...
Проверено по собранному фронту 6.1 (`extractExtraMetrics`, `sortOperator`,
`pivotOperator`): порядок столбцов задаёт пост-обработка и включается, только
если мера сортировки есть среди мер запроса, а сам топ-N режется в SQL по
`timeseries_limit_metric`. Поэтому «структура себестоимости» (`comm-08`)
сортируется и отбирается по отдельной мере `cost_sum` (сумма себестоимости):
она попадает в запрос, но не в стек. Сортировать по одной статье нельзя —
порядок по «основным материалам» не совпадает с порядком по себестоимости
(ДАНЧУК Е.Ю.: 2-й по себестоимости, 10-й по основным материалам, всё в вязании).
